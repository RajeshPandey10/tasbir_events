...
async function getFeaturedItems(): Promise<GalleryItem[]> {
  try {
...
export default async function FeaturedGallery() {
  ...
  const fallbackEntries = Object.entries(CATEGORY_IMAGES).slice(0, 6);
    ...
    <Section tone="blush">
      ...
      <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-3">
        {items.length > 0
          ...
          : fallbackEntries.map(([category, src], index) => (
              <Reveal
                key={category}
                delay={(index % 3) * 0.08}
                className="relative aspect-square overflow-hidden rounded-xl"
